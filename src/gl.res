...
module File = {
  ...
  let readFile = (~filename, ~cb) => {
    ...
    onreadystatechange(rawFile, () =>
      if getReadyState(rawFile) === 4 && (getStatus(rawFile) === 200 || getStatus(rawFile) === 0) {
...
module Window: WindowT = {
  ...
  let getPixelWidth = ((window, _ac)) => getCanvasWidth(window)
  let getPixelHeight = ((window, _ac)) => getCanvasHeight(window)
  ...
  let setWindowTitle = (~window as _, ~title) => Document.setTitle(Document.document, title)
...
let render = (
  ...
  | Some(cb) =>
    ...
    Document.addEventListener(canvas, "mousedown", e => {
      let button = switch getButton(e) {
      | 0 => Events.LeftButton
  ...
  | Some(cb) =>
    ...
    Document.addEventListener(canvas, "mouseup", e => {
      let button = switch getButton(e) {
      | 0 => Events.LeftButton
      | 1 => Events.MiddleButton
      | 2 => Events.RightButton
  ...
  | Some(cb) =>
    ...
    Document.addEventListener(canvas, "mousemove", e => {
      let rect = getBoundingClientRect(canvas)
      let x = getClientX(e) - getLeft(rect)
      let y = getClientY(e) - getTop(rect)
      cb(~x, ~y)
  ...
  let keyLastPressed = ref(list{})
  switch keyDown {
  | None => ()
  | Some(cb) => Document.addEventListener(Document.window, "keydown", e => {
      let keycode = Int32.of_int(getWhich(e))
      let repeat = List.fold_left((acc, k) => acc || k === keycode, false, keyLastPressed.contents)
      if !repeat {
        keyLastPressed := list{keycode, ...keyLastPressed.contents}
      }
  ...
  switch keyUp {
  | None => ()
  | Some(cb) => Document.addEventListener(Document.window, "keyup", e => {
      let keycode = Int32.of_int(getWhich(e))
      keyLastPressed := List.filter(k => k !== keycode, keyLastPressed.contents)
...
let shaderSource = (~context, ~shader, ~source) =>
  _shaderSource(~context, ~shader, ~source="#version 100 \n precision highp float; \n" ++ source)
@bs.send
external compileShader: (~context: contextT, shaderT) => unit = "compileShader"
@bs.send
external attachShader: (~context: contextT, ~program: programT, ~shader: shaderT) => unit =
  "attachShader"
@bs.send
external deleteShader: (~context: contextT, shaderT) => unit = "deleteShader"
@bs.send
...
@bs.send
external bindTexture: (~context: contextT, ~target: int, ~texture: textureT) => unit = "bindTexture"
@bs.send
external texParameteri: (~context: contextT, ~target: int, ~pname: int, ~param: int) => unit =
  "texParameteri"
type framebufferT
@bs.send
external createFramebuffer: (~context: contextT) => framebufferT = "createFramebuffer"
@bs.send
external bindFramebuffer: (~context: contextT, ~target: int, ~framebuffer: framebufferT) => unit =
  "bindFramebuffer"
@bs.send
external bindDefaultFramebuffer: (~context: contextT, ~target: int, @bs.as(json`null`) _) => unit =
  "bindFramebuffer"
@bs.send
...
@bs.new
external createFloat32ArrayOfArray: array<float> => 'flot32array = "Float32Array"
@bs.new external createFloat32Array: int => 'float32array = "Float32Array"
@bs.new
external createFloat64ArrayOfArray: array<float> => 'flot64array = "Float64Array"
...
@bs.new
external createInt32ArrayOfArray: array<int32> => 'int32array = "Int32Array"
@bs.new external createIntArray: int => 'int32array = "Int32Array"
@bs.new external createInt32Array: int => 'int32array = "Int32Array"
@bs.new
external createUint16ArrayOfArray: array<int> => 'uint16array = "Uint16Array"
...
module Bigarray = {
  ...
  @bs.send
  external unsafe_blit: (t<'a, 'b>, t<'a, 'b>, ~offset: int) => unit = "set"
  let unsafe_blit: (t<'a, 'b>, t<'a, 'b>, ~offset: int, ~numOfBytes: int) => unit = (
    arr,
    arr2,
    ~offset,
...
@bs.send
external addEventListener: (imageT, string, unit => unit) => unit = "addEventListener"
...
module Mat4: Mat4T = {
  ...
  @bs.scope("mat4") @bs.module("gl-matrix")
  external translate: (~out: t, ~matrix: t, ~vec: array<float>) => unit = "translate"
  @bs.scope("mat4") @bs.module("gl-matrix")
  external scale: (~out: t, ~matrix: t, ~vec: array<float>) => unit = "scale"
  @bs.scope("mat4") @bs.module("gl-matrix")
  external rotate: (~out: t, ~matrix: t, ~rad: float, ~vec: array<float>) => unit = "rotate"
  @bs.scope("mat4") @bs.module("gl-matrix")
  external ortho: (
    ~out: t,
    ~left: float,
  ...
  @bs.scope("mat4") @bs.module("gl-matrix")
  external perspective: (~out: t, ~fovy: float, ~aspect: float, ~near: float, ~far: float) => unit =
    "perspective"
  @bs.scope("mat4") @bs.module("gl-matrix")
  external lookAt: (~out: t, ~eye: array<float>, ~center: array<float>, ~up: array<float>) => unit =
    "lookAt"
}
@bs.send
external uniform1i: (~context: contextT, ~location: uniformT, ~value: int) => unit = "uniform1i"
@bs.send
external uniform1f: (~context: contextT, ~location: uniformT, ~value: float) => unit = "uniform1f"
@bs.send
...
type rec shaderParamsInternalT<'a> =
...
type rec programParamsInternalT<'a> =
...
@bs.send
external getShaderInfoLog: (~context: contextT, shaderT) => string = "getShaderInfoLog"
@bs.send
external getProgramInfoLog: (~context: contextT, programT) => string = "getProgramInfoLog"
@bs.send
external getShaderSource: (~context: contextT, shaderT) => string = "getShaderSource"
@bs.send
